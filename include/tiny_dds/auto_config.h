...
struct EntityKey {
  ...
  bool operator==(const EntityKey& other) const {
    return participant_name == other.participant_name && entity_name == other.entity_name;
  }
};
...
class AutoConfigLoader {
 public:
   ...
   */
  std::shared_ptr<Publisher> GetPublisher(const std::string& participant_name,
   ...
   */
  std::shared_ptr<Subscriber> GetSubscriber(const std::string& participant_name,
   ...
   */
  std::shared_ptr<Topic> GetTopic(const std::string& participant_name,
 ...
 private:
  ...
  // Creates a publisher
  std::shared_ptr<Publisher> CreatePublisher(std::shared_ptr<DomainParticipant> participant,
                                             const config::PublisherConfig& config);

  // Creates a subscriber
  std::shared_ptr<Subscriber> CreateSubscriber(std::shared_ptr<DomainParticipant> participant,
                                               const config::SubscriberConfig& config);

  // Creates a topic
  std::shared_ptr<Topic> CreateTopic(std::shared_ptr<DomainParticipant> participant,
