...
auto TransportManager::CreateTransport(DomainId domain_id, const std::string& participant_name,
    ...
    switch (transport_type) {
        ...
        case TransportType::SHARED_MEMORY: {
            auto shm_transport = SharedMemoryTransport::Create(
                domain_id, participant_name, buffer_size, max_message_size);
            if (!shm_transport || !shm_transport->Initialize()) {
                std::cerr << "Failed to create shared memory transport for domain " 
                          << domain_id << std::endl;
                return false;
            }
