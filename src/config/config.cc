...
static bool ParsePublisherConfig(const YAML::Node& node, PublisherConfig& publisher,
                                 const std::vector<TopicConfig>& topics);
static bool ParseSubscriberConfig(const YAML::Node& node, SubscriberConfig& subscriber,
                                  const std::vector<TopicConfig>& topics);
static bool ParseDomainParticipantConfig(const YAML::Node& node,
...
bool ValidateConfig(const Config& config) {
  ...
  for (const auto& participant : config.participants) {
    ...
    if (participant.topics.empty()) {
      std::cerr << "Warning: Participant '" << participant.name << "' has no topics defined."
                << std::endl;
    }
    ...
    for (const auto& topic : participant.topics) {
      if (topic.name.empty()) {
        std::cerr << "Error: Topic in participant '" << participant.name << "' must have a name."
                  << std::endl;
        return false;
      }

      if (topic.type_name.empty()) {
        std::cerr << "Error: Topic '" << topic.name << "' in participant '" << participant.name
    ...
    for (const auto& publisher : participant.publishers) {
      ...
      if (publisher.topic_names.empty()) {
        std::cerr << "Warning: Publisher '" << publisher.name << "' in participant '"
                  << participant.name << "' has no topics." << std::endl;
      }
    }
    ...
    for (const auto& subscriber : participant.subscribers) {
      ...
      if (subscriber.topic_names.empty()) {
        std::cerr << "Warning: Subscriber '" << subscriber.name << "' in participant '"
                  << participant.name << "' has no topics." << std::endl;
      }
    }
  }
