...
std::shared_ptr<Publisher> AutoConfigLoader::GetPublisher(const std::string& participant_name,
...
std::shared_ptr<Topic> AutoConfigLoader::GetTopic(const std::string& participant_name,
...
bool AutoConfigLoader::CreateEntities(const config::Config& config) {
  ...
  for (const auto& participant_config : config.participants) {
    auto participant =
        DomainParticipant::Create(participant_config.domain_id, participant_config.name);
...
std::shared_ptr<Publisher> AutoConfigLoader::CreatePublisher(
    std::shared_ptr<DomainParticipant> participant, const config::PublisherConfig& config) {
  return participant->CreatePublisher();
}

std::shared_ptr<Subscriber> AutoConfigLoader::CreateSubscriber(
    std::shared_ptr<DomainParticipant> participant, const config::SubscriberConfig& config) {
  return participant->CreateSubscriber();
}

std::shared_ptr<Topic> AutoConfigLoader::CreateTopic(std::shared_ptr<DomainParticipant> participant,
